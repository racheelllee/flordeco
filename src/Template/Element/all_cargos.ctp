...
<div id="updateCargosIndex">
    ...
    <div class="ibox-content">
        ...
        <table id="cargosIndexTable" class="table table-striped table-bordered table-condensed table-hover" width="100%" data-page-length='20' data-order='[[ 1, "asc" ]]'>
            ...
            <thead>
                <tr>
                    <th scope="col" style="min-width: 100px"> <?= $this->Paginator->sort('numero', __('Number')) ?></th>
                    <th style="min-width: 140px"> <?= $this->Paginator->sort('company_id', __('Company')) ?></th>
                    <th style="min-width: 140px"> <?= $this->Paginator->sort('Customers.title', __('Customer')) ?></th>
                    <th style="min-width: 110px"> <?= $this->Paginator->sort('descripcion', __('Description')) ?></th>
                    <th style="min-width: 110px"> <?= $this->Paginator->sort('Offices.name', __('Branch')) ?></th>
                    ...
                    <?php endif; ?>
                    <th scope="col" style="min-width: 150px"><?= __('Tipo de Obra') ?></th>
        ...
        <tbody>
            <?php foreach ($cargos as $row): ?>
                <tr>
                    <td> <?= h($row->numero); ?></td>
                    <td> <?= $row->has('company') ? $row->company->name : '' ; ?></td>
                    ...
                    <td>$<?= number_format($row->indicadores['monto_total'] - $row->indicadores['facturado_total'], 2) ?></td>
    ...
    <script type="text/javascript">
        $(function () {
            ...
            $('#cargos-company-id').on('change', function(ev){
                var list = company_departments[this.value];
                $('#cargos-users').empty();
                $('#cargos-users').append(new Option('Todos', ''));
                $.each(list, function(key, value){
                    $('#cargos-users').append(new Option(value, key));
                });
            });
        });
    </script>
