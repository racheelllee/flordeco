    ...
    public function add($producto_id = null,$blank=null)
    {
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->CategoriasProductos->save($categoriasProducto)) {
                //$this->Flash->success(__('La categoria del producto se guardo.'));
                //return $this->redirect('/productos/edit/'.$categoriasProducto->producto_id);

                die(json_encode($categoriasProducto));

            } //else {
                //$this->Flash->error(__('La categoria del producto no se pudo guardar.'));
            //}
        }
        ...
        $categorias_producto = $this->CategoriasProductos->find('all', [
            'conditions' => ['CategoriasProductos.producto_id' => $producto_id], 
        ...
        $OpcionefiltrosProductos = $this->OpcionefiltrosProductos->find('list', [
            ...
            //'fields' => ['OpcionefiltrosProductos.opcionesfiltro_id', 'OpcionefiltrosProductos.id']
            ])->toArray();


        //die(debug($OpcionefiltrosProductos));
        
        /*
        ->matching(
            'Categorias.Filtros.OpcionesFiltros.OpcionefiltrosProductos', function ($q) use ($producto_id) {
            return $q->where(['OpcionefiltrosProductos.producto_id' => $producto_id]);
            })
        */
